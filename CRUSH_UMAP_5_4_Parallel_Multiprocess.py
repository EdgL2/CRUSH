...
FP_NBITS = 2048              # number of fingerprint bits (as in original code)
FP_NBYTES = FP_NBITS // 8    # 256 bytes per fingerprint
...
def compute_packed_ecfp4_bytes(smiles):
    """
    Compute ECFP4 Morgan fingerprint (2048 bits) and pack to bytes (256 bytes).
    ...
    packed = np.packbits(arr)  # length FP_NBYTES
        ...
        for mode in modes_to_reactions:
            ...
            for parent_smi, rxn_name, frag_list in best_path:
                for frag in frag_list:
                    ...
                    fp_bytes = compute_packed_ecfp4_bytes(frag)
                    if fp_bytes is None:
                        continue
